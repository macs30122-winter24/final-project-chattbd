    ...
    with open('output.csv', 'a', newline='', encoding='utf-8') as csvfile:
        ...
        for k, v in links_dict.items():
            ...
            for url in v:
                ...
                if not check:
                    if reason == 'No Description box, possibly age registriction':
                ...
                csv_writer.writerow([k, channel, title, date, transcript, comments])
                counter += 1
                print(counter)

                if counter % 20 == 0: # Every 20 URL visited, clear cookies & restart driver object
                    driver.delete_all_cookies()
                    driver.execute_script("window.localStorage.clear()")
                    driver.close()
                    driver.quit() # restarting a driver object
                    driver = webdriver.Chrome(service = service, options = options)
                    driver.implicitly_wait(20)
